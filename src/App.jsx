...
const DataEngine = {
  getYearData(store, companyId, year) {
    // Return data for the fiscal year as an array of 1 item to prevent old charts from crashing
    const pd = store?.[companyId]?.[year]?.['FY'];
    if (!pd) return [{ monthIdx:0, monthTh:'FY', monthEn:'FY', revenue:0, expense:0, cashIn:0, cashOut:0, loanBalance:0, margin:0, groups: {} }];
    const g = pd.groups || {};
    return [{
      monthIdx: 0,
      monthTh: 'FY',
      monthEn: 'FY',
      revenue: g.revenue || 0,
      expense: g.expense || (g.cogs || 0) + (g.sga || 0),
      cashIn: g.operating_cash_flow > 0 ? g.operating_cash_flow : 0,
      cashOut: g.operating_cash_flow < 0 ? Math.abs(g.operating_cash_flow) : 0,
      loanBalance: g.loan || 0,
      margin: g.revenue > 0 ? ((g.net_profit || 0) / g.revenue) : 0,
      groups: g
    }];
  },
  getAvailableYears(store, companyId) { return Object.keys(store?.[companyId]||{}).map(Number).sort(); },
  countMonths(store, companyId, year) { return store?.[companyId]?.[year]?.['FY'] ? 1 : 0; },
  yearTotal(store, companyId, year, metric) { 
    return this.getYearData(store, companyId, year)[0][metric] || 0;
  },
};
...
export default function App() {
  ...
  const refreshRemoteData = async () => {
    ...
    // MOCKUP DATA
    if (session.user.id === "mockup-user") {
      setCompanies([
  ...
  const handleBackupImport = async (payload) => {
    ...
    for (const [cid,years] of Object.entries(payload.store)) {
      if (!companies.some(item=>item.id===Number(cid))) continue;
      for (const [recordYear,months] of Object.entries(years)) {
        await upsertRecords(Number(cid),Number(recordYear),Object.values(months));
      }
    }
    await refreshRemoteData();
  };
