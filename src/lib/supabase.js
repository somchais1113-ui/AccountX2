...
const key = import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY || import.meta.env.VITE_SUPABASE_KEY;
...
export async function saveImportBatch(companyId, batchDetails, normalizedDataRows) {
  ...
  const payload = normalizedDataRows.map(row => ({
    company_id: companyId,
  ...
  await client.from("normalized_financial_data")
    .delete()
